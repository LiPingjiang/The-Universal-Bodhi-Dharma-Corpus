...
def gen_content(dirname, num, name):
    ...
    elif any(kw in name for kw in ['律', '戒', '毗奈耶', '波羅提木叉', '鼻奈耶', '毗尼']):
...
for fp in sorted(glob.glob(os.path.join(BASE, "*/原文.md"))):
    ...
    # 检查是否包含待补充/待整理
    needs_fix = ('*待补充*' in content or '*待補充*' in content or '（待整理）' in content or '（待整理）' in content)
    if not needs_fix:
        continue
